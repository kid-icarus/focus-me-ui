...
const PluginDetails: React.FC<PluginProps> = ({config, updatePluginConfig}) => {
  ...
  const DefaultDetails: React.FC<PluginProps> = () => <pre>{JSON.stringify(config, null, 2)}</pre>
  const Details = pluginDetailsMap.has(pluginName) ? pluginDetailsMap.get(pluginName) : DefaultDetails;
